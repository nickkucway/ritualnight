...
enum WEAPON_SLOTS {MACHETE, CHAINSAW, MACHINE_GUN, SHOTGUN, ROCKET_LAUNCHER}
var slots_unlocked = {
	WEAPON_SLOTS.MACHETE: true,
	WEAPON_SLOTS.CHAINSAW: false,
...
func init(_fire_point: Spatial, _bodies_to_exclude: Array):
	...
	weapons[WEAPON_SLOTS.CHAINSAW].connect("fired", self, "alert_nearby_enemies")
	for weapon in weapons:
...
func get_pickup(pickup_type, ammo):
	match pickup_type:
		...
		Pickup.PICKUP_TYPES.ROCKET_LAUNCHER_AMMO:
			weapons[WEAPON_SLOTS.ROCKET_LAUNCHER].ammo += ammo
		Pickup.PICKUP_TYPES.CHAINSAW:
			if !slots_unlocked[WEAPON_SLOTS.CHAINSAW]:
				slots_unlocked[WEAPON_SLOTS.CHAINSAW] = true
				switch_to_weapon_slot(WEAPON_SLOTS.CHAINSAW)
			weapons[WEAPON_SLOTS.CHAINSAW].ammo += ammo
	emit_ammo_changed_signal()
